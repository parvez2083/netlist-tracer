"""Regression tests for picorv32 Verilog design."""
...
@pytest.mark.slow
def test_picorv32_parsing_regression():
    """Parse picorv32 and verify parser snapshot matches baseline.

    This is the AC9 behavior preservation gate: the refactored parser
    must produce identical parser output to the original implementation.
    """
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    picorv32_path = os.path.join(repo_root, "tests/fixtures/vendored/picorv32.v")
    baseline_path = os.path.join(repo_root, "tests/fixtures/golden/picorv32_baseline.json")

    assert os.path.exists(picorv32_path), f"Fixture not found: {picorv32_path}"
    assert os.path.exists(baseline_path), f"Baseline not found: {baseline_path}"

    # Parse with new implementation
    parser = NetlistParser(picorv32_path)
...
@pytest.mark.slow
def test_picorv32_tracing_regression():
    """Trace a signal path in picorv32 and verify trace snapshot matches baseline.

    This test verifies that the tracer output matches the baseline capture.
    """
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    picorv32_path = os.path.join(repo_root, "tests/fixtures/vendored/picorv32.v")
    baseline_path = os.path.join(repo_root, "tests/fixtures/golden/picorv32_baseline.json")
    ...
    trace_output = _trace_snapshot(tracer, start_cell="picorv32", start_pin="clk")
